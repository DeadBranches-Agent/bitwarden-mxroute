...
- `server/`: Flask API that mimics the Addy.io endpoints and talks to MXRoute. Core entrypoint is `server/app.py`; tests live in `server/test_app.py`.
...
- `cd server && flask run --app app.py --host=0.0.0.0 --port=6123`: Run the API locally (add `--debug` when needed).
